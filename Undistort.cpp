...
/*	if(setting_photometricCalibration==0)
    ...
    MinimalImage<unsigned short>* vm16 = nullptr;
    MinimalImageB* vm8 = nullptr;
    cv::Mat vm = cv::imread(vignetteImage.c_str(), cv::IMREAD_GRAYSCALE);
    if (vm.depth() == CV_16U) {
        vm16 = new MinimalImage<unsigned short>(vm.cols, vm.rows);
        memcpy(vm16->data, vm.data, 2*vm.rows*vm.cols);
    } else if (vm.depth() == CV_8U) {
        vm8 = new MinimalImageB(vm.cols, vm.rows);
        memcpy(vm8->data, vm.data, vm.rows*vm.cols);
    }
