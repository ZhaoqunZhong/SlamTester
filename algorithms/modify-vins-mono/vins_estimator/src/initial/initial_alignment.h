...
class ImageFrame
{
    ...
    public:
        InertialOnlyConstraint(std::shared_ptr<IntegrationBase> imu_pre_integration,
                ...
                R_c0_bkplus1_(R_c0_bkplus1), t_c0_ck_(t_c0_ck), t_c0_ckplus1_(t_c0_ckplus1),
                g0_(g0), Tic_(Tic) {}
        ...
        Evaluate(double const *const *parameters, double *residuals, double **jacobians) const {
            ...
            if (dba.norm() > 0.1 || dbg.norm() > 0.1) {
                ...
                residual.block<3, 1>(0, 0) = pre_integration_->delta_p - Tic_ +
                                             R_c0_bk_.transpose() * R_c0_bkplus1_ * Tic_
                                             -
                                             R_c0_bk_.transpose() * (t_c0_ckplus1_ - t_c0_ck_) * s -
                                             0.5 * R_c0_bk_.transpose() * dt * dt * Rg * g0_ +
                                             vb_k * dt;
                residual.block<3, 1>(3, 0) = pre_integration_->delta_v - R_c0_bk_.transpose() *
                                                                         (R_c0_bkplus1_ *
                                                                          ...
                                                                          R_c0_bk_ * vb_k +
                                                                          Rg * g0_ * dt);
                residual.block<3, 1>(6, 0) = (pre_integration_->delta_q.inverse() *
                                              Eigen::Quaterniond(
                                                      R_c0_bk_.transpose() * R_c0_bkplus1_)).vec();
            } else {
                // linearize at last bias
                Eigen::Quaterniond corrected_delta_q =
                        pre_integration_->delta_q * Utility::deltaQ(dq_dbg * dbg);
                Eigen::Vector3d corrected_delta_v =
                        pre_integration_->delta_v + dv_dba * dba + dv_dbg * dbg;
                Eigen::Vector3d corrected_delta_p =
                        pre_integration_->delta_p + dp_dba * dba + dp_dbg * dbg;
                residual.block<3, 1>(0, 0) =
                        corrected_delta_p - Tic_ + R_c0_bk_.transpose() * R_c0_bkplus1_ * Tic_
                        - R_c0_bk_.transpose() * (t_c0_ckplus1_ - t_c0_ck_) * s -
                        0.5 * R_c0_bk_.transpose() * dt * dt * Rg * g0_ + vb_k * dt;
...
/*        Matrix<double, 9, 9> cov = pre_integration_->covariance.block<9, 9>(0, 0);
        ...
        residual = sqrt_info * residual * 1e-5;*/
            ...
            if (jacobians) {
                ...
                g0_cross_mat
                        << 0.0, -g0_.z(), g0_.y(), g0_.z(), 0.0, -g0_.x(), -g0_.y(), g0_.x(), 0.0;
                if (jacobians[0]) {
                    Eigen::Map<Eigen::Matrix<double, 9, 9, Eigen::RowMajor>> jacobian_abr_Rg(
                            jacobians[0]);
                    jacobian_abr_Rg.setZero();
                    jacobian_abr_Rg.block<3, 3>(0, 0) =
                            0.5 * R_c0_bk_.transpose() * dt * dt * Rg * g0_cross_mat;
                    jacobian_abr_Rg.block<3, 3>(3, 0) =
                            R_c0_bk_.transpose() * dt * Rg * g0_cross_mat;
                }
                if (jacobians[1]) {
                    Eigen::Map<Eigen::Matrix<double, 9, 3, Eigen::RowMajor>> jacobian_abr_vbk(
                            jacobians[1]);
                ...
                if (jacobians[2]) {
                    Eigen::Map<Eigen::Matrix<double, 9, 3, Eigen::RowMajor>> jacobian_abr_vb_kplus1(
                            jacobians[2]);
                    jacobian_abr_vb_kplus1.setZero();
                    jacobian_abr_vb_kplus1.block<3, 3>(3, 0) =
                            -R_c0_bk_.transpose() * R_c0_bkplus1_;
                }
                if (jacobians[3]) {
                    Eigen::Map<Eigen::Matrix<double, 9, 1>> jacobian_abr_s(jacobians[3]);
                    jacobian_abr_s.setZero();
                    jacobian_abr_s.block<3, 1>(0, 0) =
                            -R_c0_bk_.transpose() * (t_c0_ckplus1_ - t_c0_ck_);
                }
                if (jacobians[4]) {
                    Eigen::Map<Eigen::Matrix<double, 9, 3, Eigen::RowMajor>> jacobian_abr_ba(
                            jacobians[4]);
                ...
                if (jacobians[5]) {
                    Eigen::Map<Eigen::Matrix<double, 9, 3, Eigen::RowMajor>> jacobian_abr_bg(
                            jacobians[5]);
...
/*            LOG_EVERY_N(WARNING, vins_estimator::WINDOW_SIZE)
                << "DEBUG Inertial only optimization iteration--------------";
            LOG_EVERY_N(WARNING, vins_estimator::WINDOW_SIZE) << "DEBUG acc bias iteration "
                                                              << ba.transpose() << " norm: "
                                                              << ba.norm();
            LOG_EVERY_N(WARNING, vins_estimator::WINDOW_SIZE) << "DEBUG gyr bias iteration "
                                                              << bg.transpose() << " norm: "
                                                              << bg.norm();
            LOG_EVERY_N(WARNING, vins_estimator::WINDOW_SIZE) << "DEBUG scale iteration " << s;
            LOG_EVERY_N(WARNING, vins_estimator::WINDOW_SIZE) << "DEBUG g iteration "
                                                              << (Rg * g0_).transpose() << " norm: "
                                                              << (Rg * g0_).norm();
            LOG_EVERY_N(WARNING, vins_estimator::WINDOW_SIZE) << "DEBUG residual iteration "
                                                              << residual.norm();*/
            return true;
        }
