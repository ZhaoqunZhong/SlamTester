...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image,
    ...
    LOG(INFO) << "current acc bias: " << Bas[WINDOW_SIZE].transpose()
              << "\n" << " gyr bias: " << Bgs[WINDOW_SIZE].transpose();
    ...
    if (solver_flag == INITIAL) {
        ...
        if (frame_count == WINDOW_SIZE) {
            ...
            if (result) {
                // LOG(INFO) << "Initialization costs " << timer.lagFromStartSecond()*1e3 << " ms";
...
bool Estimator::myInitialStructure() {
    ...
    /// Find out which previous frame has the largest rotation-compensated parallax with the last frame in the window.
    {
        ...
        LOG(INFO) << "Frame 0 and frame WINDOW_SIZE corresponding feature number " << ids.size();
        for (int i = 0; i < WINDOW_SIZE; i++)
        {
            ...
            if (corres.size() > 20)
            {
                ...
                LOG(INFO) << "Not enough correspondance for relative pose calculation ";
                return false;
            }
         }
        // LOG(INFO) << "Initial structure solve relative pose l : " << l;
        if (l != 0) {
            LOG(ERROR) << "Initial structure solve relative pose l index is not 0, slide window.";
            return false;
        }
    }
...
//		LOG(INFO) <<"original translation "<<c_Translation[i].transpose();
    ...
    for (int i = 0; i < sfm_f.size(); i++)
    {
        ...
        for (int j = 0; j < int(sfm_f[i].observation.size()); j++)
        {
            ...
            problem.AddResidualBlock(cost_function, NULL, c_rotation[k], c_translation[k],
                                     sfm_f[i].position);
        }
    }
    ...
    LOG(INFO) << "visual BA result " << summary.FullReport();
    ...
    {
        kv++;
        Vs[kv] = frame_i->second.R * x.segment<3>((kv + 1) * 3);
    }
