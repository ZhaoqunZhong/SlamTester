...
/*        const auto serialized_frame_data = data_serializer_->serialize_latest_frame(image_quality_);
        ...
        {
            std::lock_guard<std::mutex> lock(mtx_processed_img_);
            if (processed_img_changed_) {
                const auto serialized_frame_data = data_serializer_->serialize_latest_frame(processed_img_, image_quality_);
                if (!serialized_frame_data.empty()) {
                    client_->emit("frame_publish", serialized_frame_data);
                }
            }
            processed_img_changed_ = false;
        }
...
void publisher::publishProcessImg(cv::Mat process_img) {
    std::lock_guard<std::mutex> lock(mtx_processed_img_);
    processed_img_changed_ = true;
    processed_img_ = process_img.clone();
}
